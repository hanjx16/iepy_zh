...
if JAVA_VERSION < 8:
    ...
    #TODO for chinese
    DOWNLOAD_URL_ZH = _STANFORD_BASE_URL + 'stanford-chinese-corenlp-2016-10-31-models.jar'
    ...
    #TODO update to newest versioin
    _CORENLP_VERSION = "stanford-corenlp-full-2018-02-27"
    DOWNLOAD_URL_ES = _STANFORD_BASE_URL + 'stanford-spanish-corenlp-2018-02-27-models.jar'
    DOWNLOAD_URL_DE = _STANFORD_BASE_URL + 'stanford-german-corenlp-2018-02-27-models.jar'
    # TODO for chinese
    DOWNLOAD_URL_ZH = _STANFORD_BASE_URL + 'stanford-chinese-corenlp-2018-02-27-models.jar'

DOWNLOAD_URL = _STANFORD_BASE_URL + _CORENLP_VERSION + ".zip"
_FOLDER_PATH = os.path.join(DIRS.user_data_dir, _CORENLP_VERSION)
COMMAND_PATH = os.path.join(_FOLDER_PATH, "corenlp.sh") #TODO for hanjx download by cmd
...
class StanfordCoreNLP:
    ...
    def command_args(self, tokenize_with_whitespace, gazettes_filepath):
        ...
        # TODO for chinese
        #ref https://github.com/stanfordnlp/CoreNLP/blob/master/itest/src/edu/stanford/nlp/pipeline/TaggerParserPosTagCompatibilityITest.java
        if lang == 'zh':
            annotators.remove('dcoref')  # not supported for german on Stanford 3.4.1
            # cmd_args += " -tokenize.language zh"
            # cmd_args += " -pos.model %s/pos-tagger/chinese-distsim/chinese-distsim.tagger" % edu_mods
            # cmd_args += " -ner.model %s/ner/chinese.misc.distsim.crf.ser.gz" % edu_mods
            # cmd_args += " -parse.model %s/srparser/chineseSR.ser.gz" % edu_mods
            # TODO props for all preperties
            cmd_args += " -props %s/StanfordCoreNLP-chinese.properties" % _FOLDER_PATH
            # cmd_args += " -tokenize.language es"
...
def download(lang='en'):
    ...
    #TODO for chinese zh
    download_urls = dict(es=DOWNLOAD_URL_ES, de=DOWNLOAD_URL_DE,zh=DOWNLOAD_URL_ZH)
